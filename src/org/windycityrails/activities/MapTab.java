...
public class MapTab extends RoboTabActivity {
    ...
    private ArrayList<Location> getLocations() {
        Ln.v(MapTab.CLASSTAG + " getLocation");
    	...
    	try {
    		...
    		if(cachedFile.exists()) {
    			Ln.i("Reading locations data from cached copy.");
    			stream = new FileInputStream(cachedFile);
    		} else {
    			Ln.i("Reading locations data from assets.");
    	        stream = getAssets().open(filename);
    		}	
		} catch (IOException e) {
        	Ln.e(e, MapTab.CLASSTAG + " " + e.getMessage());
        }
    	return new XmlParser().parseLocationResponse(stream);
    }
    
    private void setupTab(final View view, final String tag, Intent intent) {
    	Ln.v(MapTab.CLASSTAG + " setupTab : " + tag);
    ...
    private static View createTabView(final Context context, final String text) {
    	Ln.v(MapTab.CLASSTAG + " createTabView with text : " + text);
